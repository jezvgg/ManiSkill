...
@register_env("MyRoboCasa-Salting-v1", asset_download_ids=["RoboCasa"])
class MyRoboCasaSceneSalting(BaseRoboCasaScene):
    FIXTURE_SEED = 102
    ...
    def _load_scene(self, options: dict):
        ...
        bowl_path = os.path.join(asset_dir, "bowl_with_salad.glb")
        bowl_scale = [0.01, 0.01, 0.01]
        asset_pose = sapien.Pose(q=degree_to_quanterion(x=90).numpy())

        self.bowl_pos = self.counter_pos.copy()
        self.bowl_pos[0] += self.counter_size[0] / 3
        self.bowl_pos[1] -= self.counter_size[1] / 4
        self.bowl_pos[2] += self.counter_size[2] / 2

        builder = self.scene.create_actor_builder()
        builder.add_nonconvex_collision_from_file(
            filename=bowl_path, pose=asset_pose, scale=bowl_scale
        )
        builder.add_visual_from_file(
            filename=bowl_path, pose=asset_pose, scale=bowl_scale
        )
        builder.initial_pose = sapien.Pose(p=self.bowl_pos)
        self.bowl = builder.build_kinematic(name="bowl")

        self.bowl_pos[2] += get_actor_size(self.bowl)[2] / 2
        bowl_pose = Pose.create_from_pq(p=self.bowl_pos)
        self.bowl.initial_pose = bowl_pose
        self.bowl.set_pose(bowl_pose)

        salt_shaker_path = os.path.join(asset_dir, "salt_shaker.glb")
        builder = self.scene.create_actor_builder()
        builder.add_convex_collision_from_file(
            filename=salt_shaker_path, pose=asset_pose
        )
        builder.add_visual_from_file(filename=salt_shaker_path, pose=asset_pose)

        right_counter = self.fixture_placements["counter_right_main_group_Counter"]
        right_counter_pos = right_counter["pos"]
        right_counter_size = right_counter["size"]

        cup_initial_pos = right_counter_pos.copy()
        cup_initial_pos[1] -= right_counter_size[1] / 2 - 0.05
        cup_initial_pos[2] += right_counter_size[2] / 2 + 0.3
        builder.initial_pose = sapien.Pose(p=cup_initial_pos)
        self.cup = builder.build_dynamic(name="cup")

        self.cup_pos = right_counter_pos.copy()
        self.cup_pos[1] -= right_counter_size[1] / 2 - 0.05
        self.cup_pos[2] += right_counter_size[2] / 2 + get_actor_size(self.cup)[2] / 2
